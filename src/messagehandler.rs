...
impl DiscordMessageHandler for FileHandler {
    fn handle_message(&self, message: &str, user: &str, db: &mut Database) -> String {
        ...
        lazy_static! {
            static ref FIND: Regex = Regex::new(r"^!file(?:\s([[:word:]ÅÄÖåäö]+))?(?:\s([^\s]+))?$").unwrap();
        }
        ...
        if let Some(matches) = FIND.captures(&message) {
            ...
            match m.len() {
                ...
                2 => FileHandler::by_filename(matches.at(1).unwrap(), db),
            ...
            "Incorrect usage. Should be: \"!file [filename [destination]]\"".to_string()
        }
...
impl FileHandler {
    ...
    fn by_filename(filename: &str, db: &mut Database) -> String {
        db.get_exact(filename.to_string())
        .as_ref()
